...
import { EditorTopbar, FormattingToolbar, TagRow, PageTabs } from "../components/EditorChrome";
import { CodeSectionRenderer } from "../components/JournalSections";
import SectionPalette from "../components/SectionPalette";
import { useToast } from "../components/Toast";
import { codeSections } from "../data/notebookData";
...
export default function CodeEditorPage() {
  ...
  function confirmDelete() { deleteNote(noteId); navigate("/"); }
  ...
  return (
    <div className="min-h-screen bg-background">
      <EditorTopbar crumbs={["Work", "Code Notes", title]} mode="code" noteId={noteId} onDelete={handleDelete} isEditMode={isEditMode} setIsEditMode={setIsEditMode} />
      <article className="editor-page code-page">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
          ...
          {isEditMode && (
            <div className="mt-7">
              <FormattingToolbar mode="code" />
            </div>
          )}
        </motion.div>

        <div className="prose-note code-prose">
          <CodeSectionRenderer sections={sections} onRemove={removeSection} onUpdate={updateSection} onMove={moveSection} isEditMode={isEditMode} />
        </div>
      </article>
      ...
      <footer className="editor-footer">
        <div className="flex items-center gap-3">
          {showTagInput && isEditMode ? (
            ...
            <button className="flex items-center gap-2 rounded-md px-2 py-1 text-sm text-muted-foreground transition hover:bg-muted" onClick={() => setShowTagInput(true)}>
          ...
          <motion.div className="fixed inset-0 z-[200] flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            >
              ...
              <div className="flex justify-end gap-3">
                <button onClick={() => setShowDeleteConfirm(false)} className="h-10 rounded-xl border border-gray-200 px-5 text-sm font-semibold text-gray-600 hover:bg-gray-50 transition">Cancel</button>
                <button onClick={confirmDelete} className="h-10 rounded-xl bg-red-600 px-5 text-sm font-semibold text-white hover:bg-red-700 transition">Delete</button>
              </div>
            </motion.div>
          </motion.div>
        )}
